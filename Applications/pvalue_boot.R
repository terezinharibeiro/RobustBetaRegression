#*************************************************************************Description*************************************************************************************************#
# Bootstrap p-values for beta regression fit under MLE and SMLE
#***ARGUMENTS***#
# y - response variable.
...
# optimal - logical; if TRUE, the SMLE is used to calculate the p-values; if FALSE, the MLE is used to calculate the p-values. Default is TRUE.
...
kk2 <- ncol(Z)
#****fit observed sample****#
fit<- SMLE_BETA(y=y, X=X, Z=Z, qoptimal=optimal, q0=1, m=3, L=0.02, qmin=0.5, spac=0.02, method="BFGS", startV="CP", linkmu=linkmu, linkphi=linkphi)
  ...
  if(kk1>1){
    ...
    #*** Comment: This is made to generate the bootstrap response variable y under H0, i.e., under the respective coefficient equal to zero***#
           for(j in 2:kk1){
                ...
                fitx_aux <- suppressWarnings(SMLE_BETA(y=y, X=as.matrix(X[,-j]), Z=Z, qoptimal= optimal, q0=1, m=3, L=0.02, qmin=0.5, spac=0.02, method="BFGS",
  ...
  if(kk2>1){
    ...
    phihat_fitz_aux <- matrix(numeric(0), ncol=kk2-1, nrow=n) 
          for(l in 2:kk2){
               ...
               fitz_aux <- suppressWarnings(SMLE_BETA(y=y, X=X, Z=as.matrix(Z[,-l]), qoptimal= optimal, q0=1, m=3, L=0.02, qmin=0.5, spac = 0.02, method="BFGS",
               startV="CP", linkmu=linkmu, linkphi=linkphi))
  ...
  for(i in 1:B){
      if(kk1>1){
            for(j in 2:kk1){
             ...
             fitx <- suppressWarnings(SMLE_BETA(y=ygen_fitx_aux, X=X, Z=Z, qoptimal= optimal, q0=1, m=3, L=0.02, qmin=0.5, spac = 0.02, method="BFGS",
             startV="CP", linkmu=linkmu, linkphi=linkphi))
      ...
      if(kk2>1){
            for(l in 2:kk2){
             ...
             #estimate with all covariates
             fitz <- suppressWarnings(SMLE_BETA(y=ygen_fitz_aux, X=X, Z=Z, qoptimal= optimal, q0=1, m=3, L=0.02, qmin=0.5, spac = 0.02, method="BFGS",
             startV="CP", linkmu=linkmu, linkphi=linkphi))

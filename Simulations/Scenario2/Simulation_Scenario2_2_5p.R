...
sink("Results_Scenario2_2_5p.txt")

names_results1 <- c("Estimates_Scenario2_n80", "Estimates_Scenario2_n160", "Estimates_Scenario2_n320")
names_results2 <- c("SE_Estimates_Scenario2_n80", "SE_Estimates_Scenario2_n160", "SE_Estimates_Scenario2_n320")
names_results3 <- c("Qoptimal_Scenario2_n80", "Qoptimal_Scenario2_n160", "Qoptimal_Scenario2_n320")

########################################################################
#######################Global Variables#################################
########################################################################
 
VN <- c(80, 160, 320) #Sample sizes
VBETA <- c(0.8, -1.2, -1.2) #true beta values
...
x1 <- runif(40); x2 <- runif(40); z1 <- x1; z2 <- x2 
X <- matrix(c(rep(1,40), x1, x2), ncol=3, byrow=F); #regressor matrix for the mean submodel
Z <- matrix(c(rep(1,40), z1, z2), ncol=3, byrow=F); #regressor matrix for the precision submodel
...
if(perc == 0.25 || perc == 0.5 || perc == 0.75 || perc ==1) cat("Perc. Replic. MC =",perc*100,"%","\n")
 ...
 SampleG <- FunSample(n = N, mXini= X, mZini = Z, size = 40, theta = c(VBETA, VGAMA), linkmu="logit", linkphi="log")
 y <- SampleG$y; mu <- SampleG$mu; phi <- SampleG$phi
 #********contamination process************#
 PC1 <- 0.0125*N #percentage 1 of contamination
 PC2 <- 0.0125*N #percentage 2 of contamination
 ind1_C <- sort(mu, index.return=TRUE, decreasing =TRUE)$ix[1:PC2] 
 ind2_C <- sort(mu, index.return=TRUE)$ix[1:PC1]
 a_c1 <- 0.01; a_c2 <- 6.0;
 ...
 y_c <- y; y_c[ind1_C] <- yaux1_c; y_c[ind2_C] <- yaux2_c

...
sink("Results_Scenario3_2_5p.txt")

names_results1 <- c("Estimates_Scenario3_n40", "Estimates_Scenario3_n80", "Estimates_Scenario3_n160", "Estimates_Scenario3_n320")
names_results2 <- c("SE_Estimates_Scenario3_n40", "SE_Estimates_Scenario3_n80", "SE_Estimates_Scenario3_n160", "SE_Estimates_Scenario3_n320")
names_results3 <- c("Qoptimal_Scenario3_n40", "Qoptimal_Scenario3_n80", "Qoptimal_Scenario3_n160", "Qoptimal_Scenario3_n320")
...
VBETA <- c(1.0, 1.8, 1.8) #true beta values
VGAMA <- c(4.5) #true gama values
...
x1 <- runif(VN[1]); x2 <- runif(VN[1])
X <- matrix(c(rep(1,VN[1]), x1, x2), ncol=3, byrow=F); #regressor matrix for the mean submodel
Z <- matrix(c(rep(1,VN[1])), ncol=1, byrow=F); #regressor matrix for the precision submodel
...
if(perc == 0.25 || perc == 0.5 || perc == 0.75 || perc ==1) cat("Perc. Replic. MC =",perc*100,"%","\n")
 ...
 PC <- 0.025*N #percentage of contamination
 ind_C <- sort(mu, index.return=TRUE, decreasing =TRUE)$ix[1:PC] 
 mu_pc <- (2.0 - mu[ind_C])/2.0 
...
cat(" TRUE VALUES ","\n")
cat(" Sample size ",N ,"\n")
out_VTHETA <- rbind(VTHETA);colnames(out_VTHETA)=c("beta1", "beta2", "beta3", "gama1");rownames(out_VTHETA)=c("");print(out_VTHETA)
...
output_mle = rbind(out_estimates_MLE);colnames(output_mle)=c("Mean estimates","Bias","Mean SE", "Null level", "Median estimates","Median Bias","Median SE");rownames(output_mle)=c("beta1","beta2","beta3","gama1");print(output_mle)
cat(" ================================ MLE estimates with contamination ================================ \n")
output_mle_c = rbind(out_estimates_MLE_C);colnames(output_mle_c)=c("Estimates","Bias","SE", "Null level", "Median estimates","Median Bias","Median SE");rownames(output_mle_c)=c("beta1","beta2","beta3","gama1");print(output_mle_c)
cat(" ================================ SMLE estimates without contamination ================================\n")
cat(" Mean of q-optimal is ",QOPTIMAL_SMLE ,"\n")
output_smle = rbind(out_estimates_SMLE);colnames(output_smle)=c("Estimates","Bias","SE", "Null level", "Median estimates","Median Bias","Median SE");rownames(output_smle)=c("beta1","beta2","beta3","gama1");print(output_smle)
cat(" ================================ SMLE estimates with contamination ================================ \n")
cat(" Mean of q-optimal is ",QOPTIMAL_SMLE_C ,"\n")
output_smle_c = rbind(out_estimates_SMLE_C);colnames(output_smle_c)=c("Estimates","Bias","SE", "Null level", "Median estimates","Median Bias","Median SE");rownames(output_smle_c)=c("beta1","beta2","beta3","gama1");print(output_smle_c)
cat(" ================================ MDPDE estimates without contamination ================================ \n")
cat(" Mean of q-optimal is ",QOPTIMAL_MDPDE ,"\n")
output_mdpde = rbind(out_estimates_MDPDE);colnames(output_mdpde)=c("Estimates","Bias","SE", "Null level", "Median estimates","Median Bias","Median SE");rownames(output_mdpde)=c("beta1","beta2","beta3","gama1");print(output_mdpde)
cat(" ================================ MDPDE estimates with contamination ================================ \n")
cat(" Mean of q-optimal is ",QOPTIMAL_MDPDE_C ,"\n")
output_mdpde_c = rbind(out_estimates_MDPDE_C);colnames(output_mdpde_c)=c("Estimates","Bias","SE", "Null level", "Median estimates","Median Bias","Median SE");rownames(output_mdpde_c)=c("beta1","beta2","beta3","gama1");print(output_mdpde_c)
cat(" ================================ Ratio of MSE ================================ \n")
...
col.names = c("MLE_beta1", "MLE_beta2", "MLE_beta3", "MLE_gama1", "MLE_C_beta1", "MLE_C_beta2", "MLE_C_beta3", "MLE_C_gama1", "SMLE_beta1", "SMLE_beta2", "SMLE_beta3",
"SMLE_gama1","SMLE_C_beta1", "SMLE_C_beta2", "SMLE_C_beta3", "SMLE_C_gama1","MDPDE_beta1", "MDPDE_beta2", "MDPDE_beta3", "MDPDE_gama1", "MDPDE_C_beta1",
 "MDPDE_C_beta2", "MDPDE_C_beta3", "MDPDE_C_gama1"))
...
col.names = c("se_MLE_beta1", "se_MLE_beta2", "se_MLE_beta3", "se_MLE_gama1", "se_MLE_C_beta1", "se_MLE_C_beta2", "se_MLE_C_beta3", "se_MLE_C_gama1", "se_MLE_beta1",
"se_SMLE_beta2", "se_SMLE_beta3","se_SMLE_gama1", "se_SMLE_C_beta1", "se_SMLE_C_beta2","se_SMLE_C_beta3", "se_SMLE_C_gama1","se_MDPDE_beta1", "se_MDPDE_beta2",
"se_MDPDE_beta3", "se_MDPDE_gama1","se_MDPDE_C_beta1", "se_MDPDE_C_beta2","se_MDPDE_C_beta3", "se_MDPDE_C_gama1"))
